...
### Where the ladder was trained

All three rungs ran on a rented RTX PRO 4500 Blackwell (32 GB) on RunPod, driven by
`scripts/runpod_prep.sh` (a throwaway CPU pod that tokenises onto the network volume) and
`scripts/runpod_train.sh` (a GPU pod on that volume, launching `train.py` detached). The
pod is deliberately not torn down on exit, since a rung takes days; `--down` does that once
`final.pt` is safe. `scripts/pull_checkpoints.py` and `scripts/volume_s3.py` read the
volume afterwards, the latter over the S3 endpoint so a slice of a 10.8 GB token file costs
one ranged request rather than a pod lifecycle.

### Where evaluation runs

Dense evaluation, generation and the probes run on a local box with an RTX 3080 (10 GB),
64 GB RAM, bare-metal Ubuntu. Dense evaluation saturates that card; batch-1 generation
reaches roughly two thirds of its power draw and is latency-bound instead. Nothing in the
evaluation path needs rented compute.

A 10 GB card trains the 47M rung directly. The 107M and 349M rungs do not fit it.
...
|--------|--------------------|-----|
| **CUDA** (RTX PRO 4500 rented, RTX 3080 local) | bf16 autocast + FlashAttention (SDPA) + optional `torch.compile` | training and evaluation |
...
Validation sitting at or just below training loss means it is learning rather than memorizing,
which is expected, since 2000 steps covered only ~9% of one epoch. **Validation rising while
...
its coherence breaks down after a couple of sentences rather than a couple of pages. The standard
...
A long run has to survive the thing that launched it. Start it with `setsid` so it detaches
from the login session, write a marker file on completion, and wait on process exit rather
than on the marker: a stale marker left by an earlier script reports success for a run that
never happened.

```bash
setsid nohup .venv/bin/python3 src/train.py --config configs/wieszcz_47m_6b7.json \
  > ~/train.log 2>&1 < /dev/null &
tail -f ~/train.log
```

Check liveness with `pgrep -f src/train.py`. Do not use `pgrep -x python`: the interpreter
in the venv is `python3`, so an exact-name match reports a live run as dead.
...
## The matched-data ladder (`*_5b` configs): SUPERSEDED

> The ladder that carries the paper's results is **`*_6b7`**: `wieszcz_47m_6b7.json`,
> `wieszcz_107m_6b7.json`, `wieszcz_349m_6b7.json`, 51,038 steps on the frozen 6.75B-token
> build with a document-level split. The section below describes the earlier 5.40B ladder,
> whose losses were measured against a different held-out split and are not comparable with
> anything current. Kept because the design argument is the same one and it is where the
> matched-data reasoning was worked out.
